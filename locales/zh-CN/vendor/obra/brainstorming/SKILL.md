...
digraph brainstorming {
    ...
    "用户批准规格？" -> "调用 writing-plans skill" [label="已批准"];
}
```

**终点状态是调用 writing-plans。** 不得调用 frontend-design、mcp-builder 或其他实现类 skill。brainstorming 之后唯一允许调用的 skill 是 writing-plans。
...
- 不得调用其他 skill。下一步只能是 writing-plans
