---
name: llm-wiki
description: "构建并维护由 LLM 维护的个人知识库。触发：摄取来源进 wiki、查询 wiki 知识、lint 质量、'加入 wiki'、'关于 X 我知道什么'，或提到 LLM wiki / Karpathy wiki。"
version: 1.3.0
author: yugasun
license: MIT
metadata:
  tags: [wiki, knowledge-base, markdown, research, notes, personal-kb, obsidian]
...
|------|------|
| `加入 wiki：…` | 摄取 |
| `关于 X 我知道什么` | 查询（默认不写文件） |
| `lint wiki` | 质量检查（优先跑 `scripts/lint-wiki.sh`） |
...
默认在**首次摄取**时触发。检查 `raw/` 与编译知识层是否存在；只创建缺失项，永不覆盖已有文件。
...
若查询/质量检查找不到 wiki 结构：告知用户「先跑一次 ingest 以初始化 wiki」，不要自动创建。非空且无关目录勿初始化，除非用户批准。
...
- **跨多主题** → 放最相关目录；用“另请参阅”/ `[[wikilink]]` 互链。
...
**归档页永不级联更新**（时点快照）。未说明变更集时，勿盲改 10+ 页。
...
3. 更新 `index.md`；摘要前缀为 `[Archived]` 或 `[归档]`。
4. 追加 `log.md`：
...
**索引一致性** — 对比 `index.md` 与实际知识页文件（排除 `index.md`、`log.md`、`SCHEMA.md`）：

- 文件存在但不在索引 → 补条目，摘要用 `(no summary)` / `(无摘要)`；Updated 优先用文内元数据，否则用 mtime。
- 索引指向不存在文件 → 标 `[MISSING]` / `[缺失]`；不擅自删条目。

**内部链接** — 检查知识页正文与 Sources 中的链接 / `[[wikilink]]`（Raw 字段由下一项校验；index/log 由上一项处理）：

- 目标不存在 → 在知识层按同名搜索。
  - 恰好一个匹配 → 改路径/链接。
  - 零个或多个 → 报告用户。

**Raw 引用** — Raw / `sources` 中每个指向 `raw/` 的链接必须存在：
...
- 摄取会更新 `index.md` + `log.md`。归档（来自查询）会同时更新两者。质量检查更新 `log.md`（自动修索引时才改 `index.md`）。纯查询不写任何文件。
