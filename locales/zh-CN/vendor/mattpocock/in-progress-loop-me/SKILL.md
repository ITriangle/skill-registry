---
name: loop-me
description: 当用户要围绕本 workspace 中想构建的工作流进行规格追问和反复澄清时使用。 当用户要直接实现、普通问答，或主题不在当前 workspace 工作流规划内时不要用。
---

# 中文导读

- 使用场景：当用户要围绕本 workspace 中想构建的工作流进行规格追问和反复澄清时使用。
- 不适用：当用户要直接实现、普通问答，或主题不在当前 workspace 工作流规划内时不要用。
