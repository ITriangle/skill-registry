---
name: aiops-setup
description: 当目标项目首次接入 aiops bundle，需要配置 issue tracker、triage 标签、领域文档结构或 aiops.yaml 时使用。 当项目已经配置完成，或用户只是要运行某个 aiops 阶段而不是做初始化时不要用。
---

# 中文导读

- 使用场景：当目标项目首次接入 aiops bundle，需要配置 issue tracker、triage 标签、领域文档结构或 aiops.yaml 时使用。
- 不适用：当项目已经配置完成，或用户只是要运行某个 aiops 阶段而不是做初始化时不要用。
