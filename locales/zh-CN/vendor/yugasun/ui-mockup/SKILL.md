---
name: ui-mockup
description: 当用户要快速产出 UI mockup、页面草图或交互外观方案来对齐视觉方向时使用。 当用户要生产级前端实现、后端逻辑或非 UI 任务时不要用。
---

# 中文导读

- 使用场景：当用户要快速产出 UI mockup、页面草图或交互外观方案来对齐视觉方向时使用。
- 不适用：当用户要生产级前端实现、后端逻辑或非 UI 任务时不要用。
