---
name: to-issues
description: 当用户要把计划、spec 或 PRD 拆成可独立领取的垂直切片 issue 时使用。 当还没有足够清晰的计划，或用户要直接实现而不是写 issue 时不要用。
---

# 中文导读

- 使用场景：当用户要把计划、spec 或 PRD 拆成可独立领取的垂直切片 issue 时使用。
- 不适用：当还没有足够清晰的计划，或用户要直接实现而不是写 issue 时不要用。
