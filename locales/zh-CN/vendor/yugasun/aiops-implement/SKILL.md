...
description: 当用户要按 lean、tdd、prune、review 的交付阶段推进实现，并需要阶段门禁和提交前确认时使用。 当任务还处在探索、需求澄清、一次性小修，或用户不需要 aiops 交付流程时不要用。
---

# 中文导读

- 使用场景：当用户要按 lean、tdd、prune、review 的交付阶段推进实现，并需要阶段门禁和提交前确认时使用。
- 不适用：当任务还处在探索、需求澄清、一次性小修，或用户不需要 aiops 交付流程时不要用。
