---
name: aiops
description: 当用户要把一次工程任务交给 aiops 流程编排，自动识别阶段、展示步骤、追踪 flow.state.yaml 并分派子 skill 时使用。 当需求只是单点代码修改、普通问答，或用户已经明确指定某个更窄的 skill 时不要用。
---

# 中文导读

- 使用场景：当用户要把一次工程任务交给 aiops 流程编排，自动识别阶段、展示步骤、追踪 flow.state.yaml 并分派子 skill 时使用。
- 不适用：当需求只是单点代码修改、普通问答，或用户已经明确指定某个更窄的 skill 时不要用。
