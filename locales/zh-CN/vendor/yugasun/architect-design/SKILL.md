...
description: 当用户要做架构设计、模块边界、技术方案权衡或沉淀设计词汇时使用。 当用户只需要直接实现、调 bug、写测试，且不需要架构层面的方案比较时不要用。
---

# 中文导读

- 使用场景：当用户要做架构设计、模块边界、技术方案权衡或沉淀设计词汇时使用。
- 不适用：当用户只需要直接实现、调 bug、写测试，且不需要架构层面的方案比较时不要用。
